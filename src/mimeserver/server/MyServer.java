package mimeserver.server;

import com.google.gson.Gson;
import mimeserver.MimeType;
        ...
        while (true) {  //데몬 스레드이므로 메인스레드가 실행 시킨 후에, 다시 돌아옴
            ...
            new Thread(()->{
                try{
                    ...
                    String mimeType = request.readLine();    //클라가 BW해야 서버가 BR가능
                    String responseData =parser(mimeType);
//                    System.out.println("클라이언트에게서 요청이 왔습니다 : " + msg);
                    //2-2. mime 적용을 위해, 클라이언트 메시지 파싱 필요
                    parser(mimeType);
...
//                    }
    ...
    private String parser(String mimeType) {
        //마임타입에 따라 다르게 리턴해주는 메서드
        Data data =new Data("SocketStudy", "portAndIp");
        //서버는 data를 응답불가
        //json, x-www, TEXT_HTML -> enum으로 만들어서 사용
